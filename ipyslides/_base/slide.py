...
class Slide:
    def __init__(self, app, captured_output, props_dict = {}):
        ...
        if captured_output.stdout:
            self._outputs.insert(0, HTML(captured_output.stdout))
            
        self._extra_outputs = {'start': [], 'end': []}
        self._css_class = f'slide-{id(self)}'
        self._css = HTML('')
    ...
    def insert(self, index):
        ...
        if captured.stdout:
            outputs.insert(0, HTML(captured.stdout))
    ...
    def set_css(self,props_dict):
        ...
        self._css = HTML(f'<style>\n{_all_css}\n</style>')
        with suppress(BaseException): # Fist time there is no label, so avoid it
            if self.app._slidelabel == self.display_label:
                self.app._update_content(change = None)
            else:
                self.app._slidelabel = self.display_label # Take CSS effect immediately 
...
def build_slide(app, slide_number_str, props_dict = {}):
    ...
    if captured.stdout:
        _slide._outputs.insert(0, HTML(captured.stdout))
